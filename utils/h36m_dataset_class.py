...
from AAAI_Code.utils.helpers import *
...
class H36M_Data(Dataset):
    def __init__(self, file_name, train=False, joints=17, get_pca=False, normalize_func=None, get_2dgt=False, subjects=['S1', 'S5', 'S7', 'S6', 'S8']):
        ...
        if get_pca:
            self.left_pca = PCA()
            self.right_pca = PCA()
            left, right = split_data_left_right(torch.tensor(self.data['poses_2d']))
            self.left_pca.fit(np.array(left))
            self.right_pca.fit(np.array(right))
...
class MPI_INF_3DHP_Dataset(Dataset):
    def __init__(self, file_name, train=False, joints=17, get_pca=False, normalize_func=None, get_2dgt=False,
...
class H36M_Data_Original_PCA(Dataset):
    def __init__(self, file_name, train=False, joints=17, get_pca=False, normalize_func=None, get_2dgt=False, subjects=['S1', 'S5', 'S7', 'S6', 'S8']):
        self.train = train
        self.data = dict()
        self.get_2dgt = get_2dgt
